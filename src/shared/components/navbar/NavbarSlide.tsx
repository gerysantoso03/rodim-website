...
import { Product } from '@/shared/libs/data/products';
      ...
      <div className="bg-[#1F1F1F] w-full h-full p-[2.4rem] flex flex-col gap-[1.6rem] lg:max-w-[82rem]">
        ...
        <div className="flex flex-col gap-[1.6rem]">
          ...
          <div className="flex flex-col gap-[.4rem]">
            ...
            {data.map((item) => (
              <Link href={`/product-rodim/${item.id}`} key={item.id}>
                <div
                  className={`hover:bg-white/10 transition-all duration-300 ease-in-out w-full text-[1.4rem] min-h-[4.8rem] flex items-center px-[.8rem] rounded-[.8rem] ${productId === item.id ? 'bg-white/10' : 'bg-transparent'}`}
                  onClick={() => {
                    setIsOpenMenu(!isOpenMenu);
                  }}
                >
