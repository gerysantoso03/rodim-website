...
const PopupLanguage = () => {
  ...
  useEffect(() => {
    const cookieLocale = document.cookie
      .split('; ')
      .find((row) => row.startsWith('RODIM_LOCALE='))
      ?.split('=')[1];
  ...
  return (
    <div
      className={`${isOpen ? 'block' : 'hidden'} fixed inset-0 z-[9999] w-screen h-screen bg-black/64 backdrop-blur-[40px] flex justify-center items-center`}
    >
