...
const CardFeelTheDifference = ({
  ...
  return (
    <div className={`relative w-full min-h-[40.8rem] lg:min-h-[48.8rem] xl:min-h-[58rem] overflow-hidden rounded-[18px] bg-white/12 px-[2rem] py-[4rem] flex flex-col gap-[4rem] lg:p-[4rem] ${isWide && "xl:p-[6.4rem] lg:flex-row lg:items-center lg:gap-[6.4rem]"}`}>
      <div className={`px-[2rem] py-[4rem] absolute inset-0 bg-[#1F1F1F] border z-10 ${isWide && "items-center"} ${isOpen ? "flex" : "hidden"}`}>
          ...
          <div className="flex gap-[1.2rem] lg:gap-[2rem] lg:flex-[1] items-start">
            ...
            <div className="flex flex-col items-start gap-[2.4rem]">
              <div className="flex flex-col gap-[.4rem]">
                <span className="text-[4rem] font-[700] lg:text-[4.4rem]">{title}</span>
      ...
      <div className={`w-full h-[28rem] ${!isWide ? 'lg:h-[29rem]' : 'lg:h-[52rem] lg:max-w-[42.4rem] xl:h-[47.2rem] xl:max-w-[60.8rem]'} relative rounded-[1.8rem] overflow-hidden border-[.2rem] border-white/25`}>
        <Image
          src={image}
          alt=""
          width={1000}
